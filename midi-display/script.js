...
function renderNotes(notes, keySignature) {
    ...
    ctx.clear();
    // Create a new stave and draw it
    //const stave = new Vex.Flow.Stave(40, 60, 250);
    const staveWidth = 250;
    const staveIndent = 125;
    const trebleStave = new Vex.Flow.Stave(staveIndent, 100, staveWidth);
    trebleStave.addClef('treble').addKeySignature(keySignature).setContext(ctx).draw();
    const bassStave= new Vex.Flow.Stave(staveIndent, 180, staveWidth);
...
function twoScales(keySignature) {
  ...
  return {flatScale: generateChromaticScale(scale, -1, keySignatureIndex),
...
function midiNoteNumberToName(noteNumbers, keySignature) {
    ...
    for (let transposedNoteNumber of transposedNoteNumbers) {
        ...
        sharpNamesList.push(sharpNoteName + '/' + octave);
        flatNamesList.push(flatNoteName + '/' + octave);
    }
    console.log("sharpNamesList: ", sharpNamesList, "flatNmaesList: ", flatNamesList);
